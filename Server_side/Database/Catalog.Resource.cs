...
public partial class Catalog
{
    ...
    public async Task Resource_Index()
    {
        ...
        md.HasAllProperties(
           ObjectType,
           "Id,Name,ResourceType,Resource_Group,Unit,FullName,Key,Laboriousness,Memo,ExternalCode,$Icon,Actual"
           );
    ...
    [Test]
    public async Task Resource_Load()
    {
        IDataModel dm = await _dbContext.LoadModelAsync(null, "[app].[Resource.Load]", new { UserId = 1 });
        var md = new MetadataTester(dm);
        md.IsAllKeys("TRoot,TResource,TResourceType,TResource_Group,TUnit");
        md.IsItemType("TRoot", "Resource", FieldType.Object);

        const string ObjectType = "TResource";

        md.HasAllProperties(
           ObjectType,
           "Id,Name,ResourceType,Resource_Group,Unit,FullName,Key,Laboriousness,Memo,ExternalCode,Actual"
           );
